...
try:
    ...
    def propagate_to_neighbour(path,  payload = None, req = 'POST'):
        ...
        global vessel_list, node_id, failure_count, blackList
        time.sleep(1)
        ...
        for vessel_id, vessel_ip in vessel_list.items():
            if int(vessel_id) in blackList:
                if node_id == node_num-1 and int(vessel_id) == 1:
                    node_index = 2
                elif node_id == node_num-2 and int(vessel_id) == node_num-2:
                    node_index = 1
                elif int(vessel_id) == node_id + 1:
                    node_index = node_id + 2
            if int(vessel_id) == node_index:
                success = contact_vessel(vessel_ip, path, payload, req)
                ...
                if not success:
                    ...
                    #for index1 in failure_count:
                    if not vessel_id in failure_count:
                        failure_count[vessel_id] = 1
                    else:
                        failure_count[vessel_id] += 1
                    if failure_count[vessel_id] == 7:
                        blackList.append(vessel_id) 
                        print "##################BLACKLIST {}\n\n".format(vessel_id)
                    print failure_count[vessel_id]
                else:
                    if vessel_id in failure_count:
                        if failure_count[vessel_id] != 7:
                            failure_count[vessel_id] = 0

    def randNum_Election():
    ...
    def propagate_to_leader(path, payload = None, req = 'POST'):
        ...
        global vessel_list, node_id, entry_sequence_index, cm_dict, leader_id, leader_rNum, failure_count_leader
        #print "cm_dict"
        #print cm_dict
        print leader_id
        for vessel_id, vessel_ip in vessel_list.items():
            if int(vessel_id) == leader_id: # propagate to leader only
                success = contact_vessel(vessel_ip, path, payload, req)
                if not success:
                    print "\n\nSNN2 Could not contact Leader {}\n\n".format(vessel_id)
                    if not vessel_id in failure_count_leader:
                        failure_count_leader[vessel_id] = 1
                    else:
                        failure_count_leader[vessel_id] += 1
                    if failure_count_leader[vessel_id] == 7:
                        print "##################RE-ELECTION!!!!!!!!!!!!!!!!!!!!!!!!!! {}\n\n".format(vessel_id)
                        t = Thread( target = randNum_Election)
                        t.daemon = True
                        t.start()

                    print failure_count_leader[vessel_id]
                else:
                    if vessel_id in failure_count_leader:
                        if failure_count_leader[vessel_id] != 7:
                            failure_count_leader[vessel_id] = 0
    ...
    def election_message(emessage_id):
        ...
        global received_election_dict, election_dict, countEM, node_num, leader_dict, leader_id, leader_rNum
        time.sleep(1)
        body = request.body.read()
        #if countEM == 1:
        a, b = body.split('=')
        #received_election_dict = {int(a) : int(b)}
        ...
        print leader_dict
        #print election_dict
        #received_election_dict.update(election_dict)
        #print received_election_dict
        #if len(received_election_dict) == node_num - 1:
        if countEM == node_num - 1:
            ...
            print leader_rNum
            #t = Thread(target = elect_leader) #ELECT LEADER LOGIC HERE
            #t.demon = True
            #t.start()
        else:
            countEM = countEM + 1
    ...
    def main():
        ...
        global entry_sequence_index, node_num, leader_dict
        global election_dict, countEM, countCM, leader_id, leader_rNum, cm_dict, failure_count, blackList
        blackList = [0]
