...
try:
    ...
    def elect_leader():
        global leader_id, leader_rNum, cm_dict, received_election_dict, countCM
        print "elect leader"
        print received_election_dict
        try:
            leader_id = max(received_election_dict.iteritems(), key=operator.itemgetter(1))[0]
            leader_rNum = max(received_election_dict.iteritems(), key=operator.itemgetter(1))[1]
            cm_dict = {leader_id : leader_rNum}
            print leader_id
            print leader_rNum
            countCM = countCM + 1
            t = Thread( target = propagate_to_neighbour, args= ('/cm2neighbor/{}'.format(countCM), cm_dict,'POST'))
            t.daemon = True
            t.start()
            return True
        except Exception as e:
    ...
    def propagate_to_leader(path, payload = None, req = 'POST'):
        ...
        global vessel_list, node_id, entry_sequence_index, cm_dict, leader_id, leader_rNum
        print "cm_dict"
        print cm_dict
        print leader_id
        for vessel_id, vessel_ip in vessel_list.items():
    ...
    def election_message(emessage_id):
        ...
        global received_election_dict, election_dict, countEM, node_num
        time.sleep(1)
        print "In election_message function"
        body = request.body.read()
        if countEM == 1:
            a, b = body.split('=')
            ...
            for x in mylist:
                a, b = x.split('=')
                my_election_dict = {int(a) : int(b)}
                received_election_dict.update(my_election_dict)
            print received_election_dict
        countEM = countEM + 1
        if len(received_election_dict) == node_num - 1:
            ...
            t = Thread(target=propagate_to_neighbour, args =('/em2neighbor/{}'.format(countEM), received_election_dict,'POST'))
            t.daemon = True
            t.start()
    ...
    def coordination_message(cmessage_id):
        ...
        global countCM, node_num, cm_dict
        time.sleep(1)
        countCM = countCM + 1
        print countCM
        if countCM == node_num:
            print "finish coordination\n\n"
            return True
        else:
            t = Thread(target=propagate_to_neighbour, args =('/cm2neighbor/{}'.format(countCM), cm_dict,'POST'))
            t.daemon = True
    ...
    def main():
        ...
        global election_dict, countEM, countCM, leader_id, leader_rNum, cm_dict
