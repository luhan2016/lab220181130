...
try:
    ...
    def contact_vessel(vessel_id, vessel_ip, path, payload=None, req='POST'):
        ...
        try:
            ...
            print e
            print "****************node_id is dead ************************"
            for vessel_id_next, vessel_ip_next in vessel_list.items():
                if int(vessel_id) == int(node_num) -1 and int(vessel_id_next) == 1:
                    contact_vessel(vessel_id_next, vessel_ip_next, path, payload, 'POST')
                    print "****************node_id is dead ************************"
                elif int(vessel_id_next) == int(vessel_id)+ 1:
                    contact_vessel(vessel_id_next, vessel_ip_next, path, payload, 'POST')
            print "****************node_id is dead ************************"
        return success
    ...
    def random_number():
        ...
        #time.sleep(1)
        countEM = countEM + 1
        t = Thread( target = propagate_to_neighbour, args= ('/em2neighbor/{}/{}'.format(countEM, node_id), election_dict,'POST'))
        t.daemon = True
        t.start()
    ...
    @app.post('/em2neighbor/<emessage_id:int>/<previous_node_id:int>')
    def election_message(emessage_id,previous_node_id):
        print "in election message function"
        print "previous_node_id is "
        print previous_node_id
        global received_election_dict, election_dict, countEM, countCM, node_num, leader_dict, leader_id, leader_rNum, node_id
        time.sleep(1)
        body = request.body.read()
        a, b = body.split('=')
        ...
        if countEM == node_num - 1:
            ...
            countEM = countEM + 1
            t = Thread(target=propagate_to_neighbour, args =('/em2neighbor/{}/{}'.format(countEM,node_id), leader_dict,'POST'))
            t.daemon = True
            t.start()
