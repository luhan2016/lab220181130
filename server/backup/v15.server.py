...
try:
    ...
    def contact_deadnode_neighbor(vessel_id, vessel_ip, path, payload=None, req='POST'):
        ...
        for vessel_id_next, vessel_ip_next in vessel_list.items():         
            if int(dead_nodeID) == int(node_num) -1 and int(vessel_id_next) == 1:
                success = contact_vessel(vessel_id_next, vessel_ip_next, path, payload, 'POST')
    ...
    def propagate_to_vessels(path, payload = None, req = 'POST', leaderElected=False):
        ...
        for vessel_id, vessel_ip in vessel_list.items():
            if int(vessel_id) != int(node_id) : # don't propagate to yourself
                success = contact_vessel(vessel_id, vessel_ip, path, payload, req, leaderElected)
                if not success:
                    if not leaderElected:
                        print "SNN Could not contact vessel {}".format(vessel_id)
    ...
    # random number function creats random number for each node and propagate the random number with node id to neighbor
    def random_number():
    ...
    def propagate_to_leader(path, payload = None, req = 'POST'):
        ...
        for vessel_id, vessel_ip in vessel_list.items():
            if int(vessel_id) == int(leader_id): 
    ...
    # propagate received message from webpage to leader first, and the leader will update the store, then the leader will propagate received message to other vessels
    @app.post('/board')
    ...
    # when receive modify or delete message from webpage, propagate these message to leader first, then the leader will propagate to others
    ...
    # according to the received action, decide if it is add/modify/delete the entry in store
    ...
    def election_message(emessage_id):
        ...
        if countEM == node_num - 1:              #Stopping Condition for Election message
            ...
            countCM = countCM + 1
            t = Thread( target = propagate_to_neighbour, args= ('/cm2neighbor/{}'.format(countCM), leader_dict,'POST'))    #### START the Coordinate message
            t.daemon = True
            t.start()
        else:                                    #Keep propagating the Election message
            countEM = countEM + 1
    ...
    # when after adding all nodes' leader in the received_coordinate_dict, count how many elements in the dictionary
    ...
    def coordination_message(cmessage_id):
        ...
        try:
            ...
            countCM = countCM + 1
            if countCM == node_num:      ## Stopping point for coordinate message
                for key, value in received_coordinate_dict.items():      ## Check the number of Key/Value pairs in dict
                    countLeader = countLeader +1
                if countLeader == 1:                                     ## if 1 pair of values then all clients have same leader
    ...
    # entry_leader function is called after user input in the browser and then send the message to leader
    # the leader will check the action is add, modify or delete, and prpagate entry message to other vessels
    ...
    def entry_leader(action, element_id):
        ...
        try:
            if action == 'add':
                ...
                delete_element_from_store(element_id)
                t = Thread(target=propagate_to_vessels, args = ('/propagate/delete/{}'.format(element_id),None,'POST',True )) 
                t.daemon = True
                t.start()
            template('server/boardcontents_template.tpl',board_title='Vessel {}'.format(node_id), board_dict=sorted(board.iteritems()))
        except Exception as e:
    ...
    def main():
        ...
        #create a thread to generate random number for each node and compare the random number
